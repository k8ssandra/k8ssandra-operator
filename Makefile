...
KIND_KUBECONFIG ?= ./build/kubeconfig
...
single-up: cleanup build manifests kustomize docker-build create-kind-cluster kind-load-image cert-manager
	...
	kubectl config use-context k8ssandra-0
...
##install the data plane
	for ((i = 1; i < $(NUM_CLUSTERS); ++i)); do \
		kubectl config use-context k8ssandra-$$i; \
...
cleanup:
	for ((i = 0; i < $(NUM_CLUSTERS); ++i)); do \
		...
		kubectl config delete-context k8ssandra-$$i || true; \
	done

create-kind-cluster:
	scripts/setup-kind-multicluster.sh --clusters 1 --kind-worker-nodes 4 --overwrite --output-file $(KIND_KUBECONFIG)

create-kind-multicluster:
	scripts/setup-kind-multicluster.sh --clusters $(NUM_CLUSTERS) --kind-worker-nodes 4 --overwrite --output-file $(KIND_KUBECONFIG)
...
create-clientconfig:
	for ((i = 0; i < $(NUM_CLUSTERS); ++i)); do \
		scripts/create-clientconfig.sh \
		  ...
		  --src-context k8ssandra-$$i \
		  --dest-context k8ssandra-0; \
	done
