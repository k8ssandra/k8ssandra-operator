...
func TestGetTargetRackFQDNs(t *testing.T) {
	...
	assert.NoError(t, err, err)
	expectedSourceRacks := map[NodeLocation][]string{
		{Rack: "default", DC: "dc2"}: {"test-cluster-dc2-default-sts-0", "test-cluster-dc2-default-sts-1", "test-cluster-dc2-default-sts-2"},
	}
	...
	expectedSourceRacks = map[NodeLocation][]string{
		{Rack: "rack1", DC: "dc2"}: {"test-cluster-dc2-rack1-sts-0"},
		{Rack: "rack2", DC: "dc2"}: {"test-cluster-dc2-rack2-sts-0"},
		{Rack: "rack3", DC: "dc2"}: {"test-cluster-dc2-rack3-sts-0"},
	}
	result, err = getTargetRackFQDNs(cassDc)
	assert.NoError(t, err, err)
	assert.Equal(t, expectedSourceRacks, result)
