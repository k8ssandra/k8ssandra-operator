...
func TestPutMapNested(t *testing.T) {
	tests := []struct {
		...
		{
			"partially existing keys overlap no overwrite",
			false,
		...
		{
			"partially existing keys overlap no overwrite 2",
			false,
			map[string]interface{}{"foo": map[string]interface{}{"bar": nil}},
			[]string{"foo", "bar", "qix"},
			map[string]interface{}{"foo": map[string]interface{}{"bar": map[string]interface{}{"qix": 42}}},
			assert.NoError,
		},
		{
			"partially existing keys overlap no overwrite 3",
			false,
