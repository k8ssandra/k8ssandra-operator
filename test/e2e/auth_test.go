...
func multiDcAuthOnOff(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	reaper1Key := framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "cluster1-dc1-reaper"}}
	reaper2Key := framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "cluster1-dc2-reaper"}}

	stargate1Key := framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "cluster1-dc1-stargate"}}
	stargate2Key := framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "cluster1-dc2-stargate"}}
	...
	f.DeployStargateIngresses(t, *dataPlane1, 0, namespace, "cluster1-dc1-stargate-service", "", "")
	f.DeployStargateIngresses(t, *dataPlane2, 1, namespace, "cluster1-dc2-stargate-service", "", "")
	f.DeployReaperIngresses(t, ctx, *dataPlane1, 0, namespace, "cluster1-dc1-reaper-service")
	f.DeployReaperIngresses(t, ctx, *dataPlane2, 1, namespace, "cluster1-dc2-reaper-service")
	defer f.UndeployAllIngresses(t, *dataPlane1, namespace)
	defer f.UndeployAllIngresses(t, *dataPlane2, namespace)
...
func checkSecrets(
	...
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: superuserSecretKey})
	checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: superuserSecretKey})
	if expectReaperSecretsCreated {
		t.Log("check that reaper CQL secret exists in both contexts")
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperCqlSecretKey})
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperCqlSecretKey})
		t.Log("check that reaper JMX secret exists in both contexts")
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperJmxSecretKey})
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperJmxSecretKey})
		t.Log("check that reaper UI secret exists in both contexts")
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperUiSecretKey})
		checkSecretExists(t, f, ctx, kcKey, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperUiSecretKey})
	} else {
		t.Log("check that reaper CQL secret wasn't created in neither context")
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperCqlSecretKey})
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperCqlSecretKey})
		t.Log("check that reaper JMX secret wasn't created in neither context")
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperJmxSecretKey})
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperJmxSecretKey})
		t.Log("check that reaper UI secret wasn't created in neither context")
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperUiSecretKey})
		checkSecretDoesNotExist(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: reaperUiSecretKey})
	}
}
...
func waitForAllComponentsReady(
	...
	options1 := kubectl.Options{Namespace: kcKey.Namespace, Context: *dataPlane1}
	options2 := kubectl.Options{Namespace: kcKey.Namespace, Context: *dataPlane2}
...
func testAuthenticationDisabled(
	...
	t.Run("TestJmxAccessAuthDisabled", func(t *testing.T) {
		t.Run("Local", func(t *testing.T) {
			...
			checkNodeToolStatus(t, f, *dataPlane1, namespace, pod1Name, 2, 0)
			checkNodeToolStatus(t, f, *dataPlane2, namespace, pod2Name, 2, 0)
		})
		t.Run("Remote", func(t *testing.T) {
			...
			checkNodeToolStatus(t, f, *dataPlane1, namespace, pod1Name, 2, 0, "-h", pod2IP)
			checkNodeToolStatus(t, f, *dataPlane2, namespace, pod2Name, 2, 0, "-h", pod1IP)
		})
	})
...
func testAuthenticationEnabled(
	...
	t.Run("TestJmxAccessAuthEnabled", func(t *testing.T) {
		t.Run("Local", func(t *testing.T) {
			...
			checkNodeToolStatus(t, f, *dataPlane1, namespace, pod1Name, 2, 0, "-u", username, "-pw", password)
			checkNodeToolStatus(t, f, *dataPlane2, namespace, pod2Name, 2, 0, "-u", username, "-pw", password)
			checkLocalJmxFailsWithNoCredentials(t, f, *dataPlane1, namespace, pod1Name)
			checkLocalJmxFailsWithNoCredentials(t, f, *dataPlane2, namespace, pod2Name)
			checkLocalJmxFailsWithWrongCredentials(t, f, *dataPlane1, namespace, pod1Name)
			checkLocalJmxFailsWithWrongCredentials(t, f, *dataPlane2, namespace, pod2Name)
		})
		t.Run("Remote", func(t *testing.T) {
			t.Log("check that nodes in different dcs can see each other (auth enabled, remote JMX)")
			pod1IP, pod2IP := getPodIPs(t, f, namespace, pod1Name, pod2Name)
			checkNodeToolStatus(t, f, *dataPlane1, namespace, pod1Name, 2, 0, "-h", pod2IP, "-u", username, "-pw", password)
			checkNodeToolStatus(t, f, *dataPlane2, namespace, pod2Name, 2, 0, "-h", pod1IP, "-u", username, "-pw", password)
			checkRemoteJmxFailsWithNoCredentials(t, f, *dataPlane1, namespace, pod1Name, pod2IP)
			checkRemoteJmxFailsWithNoCredentials(t, f, *dataPlane2, namespace, pod2Name, pod1IP)
			checkRemoteJmxFailsWithWrongCredentials(t, f, *dataPlane1, namespace, pod1Name, pod2IP)
			checkRemoteJmxFailsWithWrongCredentials(t, f, *dataPlane2, namespace, pod2Name, pod1IP)
		})
	})
	t.Run("TestApisAuthEnabled", func(t *testing.T) {
		t.Run("Stargate", func(t *testing.T) {
			testStargateApis(t, ctx, *dataPlane1, 0, username, password, replication)
			testStargateApis(t, ctx, *dataPlane2, 1, username, password, replication)
		...
		t.Run("Reaper", func(t *testing.T) {
			username, password := retrieveCredentials(t, f, ctx, framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: reaperUiSecretKey})
...
func getPodIPs(t *testing.T, f *framework.E2eFramework, namespace, pod1Name, pod2Name string) (string, string) {
	pod1IP, err := f.GetPodIP(*dataPlane1, namespace, pod1Name)
	require.NoError(t, err, "failed to get pod %s IP in context %s", pod1Name, *dataPlane1)
	pod2IP, err := f.GetPodIP(*dataPlane2, namespace, pod2Name)
	require.NoError(t, err, "failed to get pod %s IP in context %s", pod2Name, *dataPlane2)
