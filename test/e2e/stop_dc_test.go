...
func stopAndRestartDc(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	dc1Key := framework.NewClusterKey(*dataPlane1, namespace, "dc1")
	sg1Key := framework.NewClusterKey(*dataPlane1, namespace, "cluster1-dc1-stargate")
	reaper1Key := framework.NewClusterKey(*dataPlane1, namespace, "cluster1-dc1-reaper")

	dc2Key := framework.NewClusterKey(*dataPlane2, namespace, "dc2")
	sg2Key := framework.NewClusterKey(*dataPlane2, namespace, "cluster1-dc2-stargate")
	reaper2Key := framework.NewClusterKey(*dataPlane2, namespace, "cluster1-dc2-reaper")
	...
	t.Log("deploying Stargate and Reaper ingress routes in", *dataPlane2)
	f.DeployStargateIngresses(t, *dataPlane2, 1, namespace, "cluster1-dc2-stargate-service", username, password)
	...
	checkKeyspaceReplicationsUnaltered(t, f, ctx, *dataPlane2, namespace, pod2Name)

	t.Run("TestApisDc1Stopped", func(t *testing.T) {
		testStargateApis(t, ctx, *dataPlane2, 1, username, password, map[string]int{"dc2": 1})
		uiKey := framework.NewClusterKey(*dataPlane2, namespace, reaper.DefaultUiSecretName("cluster1"))
		uiUsername, uiPassword := retrieveCredentials(t, f, ctx, uiKey)
		connectReaperApi(t, ctx, 1, "cluster1", uiUsername, uiPassword)
		checkNodeToolStatus(t, f, *dataPlane2, namespace, pod2Name, 1, 1, "-u", username, "-pw", password)
	})
	...
	t.Log("deploying Stargate and Reaper ingress routes in", *dataPlane1)
	f.DeployStargateIngresses(t, *dataPlane1, 0, namespace, "cluster1-dc1-stargate-service", username, password)
	f.DeployReaperIngresses(t, ctx, *dataPlane1, 0, namespace, "cluster1-dc1-reaper-service")

	checkKeyspaceReplicationsUnaltered(t, f, ctx, *dataPlane1, namespace, pod1Name)

	t.Run("TestApisDc2Stopped", func(t *testing.T) {
		testStargateApis(t, ctx, *dataPlane1, 0, username, password, map[string]int{"dc1": 1})
		uiKey := framework.NewClusterKey(*dataPlane1, namespace, reaper.DefaultUiSecretName("cluster1"))
		uiUsername, uiPassword := retrieveCredentials(t, f, ctx, uiKey)
		connectReaperApi(t, ctx, 0, "cluster1", uiUsername, uiPassword)
		checkNodeToolStatus(t, f, *dataPlane1, namespace, pod1Name, 1, 1, "-u", username, "-pw", password)
	})
	...
	t.Run("TestApisDcsRestarted", func(t *testing.T) {
		testStargateApis(t, ctx, *dataPlane1, 0, username, password, map[string]int{"dc1": 1, "dc2": 1})
		testStargateApis(t, ctx, *dataPlane2, 1, username, password, map[string]int{"dc1": 1, "dc2": 1})
		uiKey := framework.NewClusterKey(*dataPlane1, namespace, reaper.DefaultUiSecretName("cluster1"))
		uiUsername, uiPassword := retrieveCredentials(t, f, ctx, uiKey)
