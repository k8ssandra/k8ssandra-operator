...
func multiDcMultiCluster(t *testing.T, ctx context.Context, klusterNamespace string, f *framework.E2eFramework) {
	...
	dc1Key := framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: types.NamespacedName{Namespace: dc1Namespace, Name: "dc1"}}
	...
	dc2Key := framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: types.NamespacedName{Namespace: dc2Namespace, Name: "dc2"}}
	...
	count := 6
	checkNodeToolStatus(t, f, *dataPlane1, dc1Namespace, pod, count, 0, "-u", username, "-pw", password)

	t.Log("check nodes in dc2 see nodes in dc1")
	pod = "test-dc2-rack1-sts-0"
	checkNodeToolStatus(t, f, *dataPlane2, dc2Namespace, pod, count, 0, "-u", username, "-pw", password)
}
