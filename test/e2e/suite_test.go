...
var (
	...
	testProfileFlag = flag.String(
		"testProfile",
		"kind-local",
		"The test profile to use. Supported profiles: kind-local (default), gke.",
	)
...
func TestOperator(t *testing.T) {
	...
	t.Run("RemoveTemplating", e2eTest(ctx, &e2eTestOpts{
		testFunc: func(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
			t.Logf("Running test fixture in namespace %s", namespace)
			...
			username, password, err := f.RetrieveDatabaseCredentials(ctx, f.DataPlaneContexts[0], namespace, "cluster1")
			require.NoError(t, err)
			testStargateApis(t, ctx, f.DataPlaneContexts[0], username, password, map[string]int{"dc1": 3, "dc2": 3})
			testStargateApis(t, ctx, f.DataPlaneContexts[1], username, password, map[string]int{"dc1": 3, "dc2": 3})
...
type e2eTestOpts struct {
	...
	// fixture specifies the fixture to use.
	fixture string
...
func beforeTest(t *testing.T, f *framework.E2eFramework, opts *e2eTestOpts) error {
	...
	if err := f.DeployK8ssandraOperator(opts.operatorNamespace, *testProfileFlag, opts.clusterScoped); err != nil {
	...
	if opts.fixture != "" {
		if err := f.DeployFixture(opts.fixture, *testProfileFlag); err != nil {
			t.Logf("failed to deploy fixture")
			return err
		}
		if err := createIngressConfigs(opts.sutNamespace); err != nil {
			t.Logf("failed to create ingress configs")
			return err
		}
	}
...
func processFlags(t *testing.T) {
	...
	if len(*testProfileFlag) == 0 {
		t.Fatal("no test profile provided")
	} else {
		switch *testProfileFlag {
		case "kind-local":
		case "gke":
		default:
			t.Fatalf("invalid test profile: %s", *testProfileFlag)
		}
	}
}

func createIngressConfigs(sutNamespace string) error {
	ingressConfigs = make(map[string]ingressConfig)
	if *testProfileFlag == "kind-local" {
		for _, name := range dataPlanes {
			prefix := name[strings.LastIndex(name, "-")+1:]
			ingressConfigs[name] = ingressConfig{
				StargateRest: framework.HostAndPort(fmt.Sprintf("stargate.%v.127.0.0.1.nip.io:3%v080", sutNamespace, prefix)),
				StargateCql:  framework.HostAndPort(fmt.Sprintf("stargate.%v.127.0.0.1.nip.io:3%v942", sutNamespace, prefix)),
				ReaperRest:   framework.HostAndPort(fmt.Sprintf("reaper.%v.127.0.0.1.nip.io:3%v080", sutNamespace, prefix)),
			}
		}
	} else {
		for _, dataPlane := range dataPlanes {
			...
			ip, err := kubectl.Get(options, "svc", "traefik", "-o", "jsonpath='{.status.loadBalancer.ingress[0].ip}'")
			if err != nil {
				return err
			} else if ip == "" {
				return fmt.Errorf("load balancer external IP not found for context %v", dataPlane)
			}
			ip = strings.Trim(ip, "'")
			ingressConfigs[dataPlane] = ingressConfig{
				StargateRest: framework.HostAndPort(fmt.Sprintf("stargate.%v.%v.nip.io:8080", sutNamespace, ip)),
				StargateCql:  framework.HostAndPort(fmt.Sprintf("stargate.%v.%v.nip.io:9042", sutNamespace, ip)),
				ReaperRest:   framework.HostAndPort(fmt.Sprintf("reaper.%v.%v.nip.io:8080", sutNamespace, ip)),
			}
		}
	}
	return nil
