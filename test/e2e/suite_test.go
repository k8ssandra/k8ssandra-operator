...
var (
	...
	// FIXME while data plane context names are configurable in test code, they are currently not configurable in
	// fixture files. Before we can actually change the default values below, we'll need to address that first.
	dataPlane1 = flag.String(
		"dataPlane1",
		"k8ssandra-0",
		"The name of a Kubernetes context to use as the first data plane context",
	)
	dataPlane2 = flag.String(
		"dataPlane2",
		"k8ssandra-1",
		"The name of a Kubernetes context to use as the second data plane context",
	)
...
func e2eTest(ctx context.Context, opts *e2eTestOpts) func(*testing.T) {
	return func(t *testing.T) {
		...
		f, err := framework.NewE2eFramework(t, filepath.Join("..", "..", *configFile), *controlPlane, *dataPlane1, *dataPlane2)
		if err != nil {
...
func beforeTest(t *testing.T, f *framework.E2eFramework, opts *e2eTestOpts) error {
	...
	if err := f.DeployK8sClientConfigs(
...
func createSingleDatacenterCluster(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	stargateKey := framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc1-stargate"}}
	...
	t.Log("deploying Stargate ingress routes in context", *dataPlane1)
...
func createStargateAndDatacenter(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	stargateKey := framework.ClusterKey{K8sContext: *dataPlane1, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "s1"}}
	checkStargateReady(t, f, ctx, stargateKey)
...
func createMultiDatacenterCluster(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	count := 6
	checkNodeToolStatus(t, f, *dataPlane1, namespace, pod, count, 0, "-u", username, "-pw", password)
	...
	checkNodeToolStatus(t, f, *dataPlane2, namespace, pod, count, 0, "-u", username, "-pw", password)
...
func addDcToCluster(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	_, err = f.ExecuteCql(ctx, *dataPlane1, namespace, "test", "test-dc1-default-sts-0",
	...
	for _, ks := range keyspaces {
		assert.Eventually(func() bool {
			output, err := f.ExecuteCql(ctx, *dataPlane1, namespace, "test", "test-dc1-default-sts-0",
...
func checkStargateApisWithMultiDcCluster(t *testing.T, ctx context.Context, namespace string, f *framework.E2eFramework) {
	...
	stargateKey = framework.ClusterKey{K8sContext: *dataPlane2, NamespacedName: types.NamespacedName{Namespace: namespace, Name: "test-dc2-stargate"}}
	checkStargateReady(t, f, ctx, stargateKey)
	...
	t.Log("deploying Stargate ingress routes in context", *dataPlane2)
